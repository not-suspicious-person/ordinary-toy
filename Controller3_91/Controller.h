...
uint16_t read16(File& f);
  ...
  int16_t                 _desired_state_cam[ARRAY_SIZE]      = {0};
...
//  menu_buttons            _current_menu_button                = _8;
  ...
  uint16_t*               _cam_scaled_row                     = nullptr;
  ...
  static const int        CAM_PIXELS                          = SCREEN_WIDTH * SCREEN_HEIGHT;
  uint16_t*               _camRgb565Buf                       = nullptr;
  ...
  void                    set_CS(uint8_t cs_pin, bool active);
  void                    display_Data();
  void                    read_Encoders();
  ...
  void                    startSoftAP(uint8_t channel = 1, uint16_t port = 80); 
  void                    stopSoftAP(); 
  void                    handleCameraTCP();
  void                    startCameraStream(uint16_t port = 80); 
  void                    stopCameraStream();
  bool                    isAllZeroMac(const uint8_t* mac);
